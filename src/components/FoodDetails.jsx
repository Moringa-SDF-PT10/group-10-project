...
useEffect(() => {
  const fetchFoodsWithRandomImages = async () => {
    try {
      ...
      const foodResponse = await fetch(`https://api.nal.usda.gov/fdc/v1/food/${fdcId}?api_key=8Rv57kYxBTe6DURXfAZu8DBKcol1W0hpsrc7d1xJ`);
   ...
   // 2. Fetch meal data from MealDB
      ...
      const enrichedFoods = { ...foodData,  image: randomMeal.strMealThumb || "https://via.placeholder.com/500x600?text=No+Image"
        }; 
...
<div id = "detail-nutrient-container">
  ...
  {food.foodNutrients && food.foodNutrients.map((nutrient, index) => (
    <li key={index}>
      <span className="nutrient-name">{nutrient.nutrient.name}</span>
        <span className="nutrient-name">{nutrient.nutrient.name}</span>
      <span className="nutrient-details">
        {nutrient.nutrient.amount} {nutrient.nutrient.unitName}
      </span>
    </li>
  ))}
</ul>
